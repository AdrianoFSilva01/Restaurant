<template>
    <div>
        <ContextMenu
            ...
            :open-to-left="true"
        >
            <div class="bg-black bg-opacity-50 border border-white" :class="{ 'selected' : store.state.selectedCategories.get(categoryIndex)?.has(selectedIngredients) }">
                <div class="bg-white text-2xl flex justify-center px-6 py-1" :class="{ 'bg-gold' : store.state.selectedCategories.get(categoryIndex)?.has(selectedIngredients) }">
                    Ingredients
                </div>
                <div class="px-2 py-1">
                    ...
                    <div class="animation-container" @animationend="onAnimationEnd">
                        <div class="catalog" style="tap-highlight-color: transparent" @click="addCatalog(catalog.id, catalog.catalogInfos[0].size, 1)" :class="{ 'selected' : store.state.selectedCategories.get(categoryIndex)?.has(catalog.id) }">
                            <template v-if="catalog.haveIngredients">
                                ...
                                >
                                    <div class="text-black m-0.5 p-0.5 bg-white w-6 relative pointer-events-none rounded" :class="{ 'bg-gold' : store.state.selectedCategories.get(categoryIndex)?.has(catalog.id) }">
                                ...
                                <template v-if="catalog.catalogInfos.length > 1">
                                    ...
                                    >
                                        <template v-for="[size, quantity] in getCatalogsInfo(catalog.id)" :key="size">
                                            <div class="flex space-x-px py-0.5 px-1.5 rounded text-black"
                                        ...
                                        <template v-for="(info, index) in catalog.catalogInfos" :key="index">
                                            <div class="flex flex-grow space-x-1">
                                                <div class="size bg-white"
                                                     :class="{
                                                         'bg-darkGold lg:bg-gold' : store.state.selectedCategories.get(categoryIndex)?.get(catalog.id)?.has(info.size) && store.state.selectedCatalogs.get(catalog.id) !== info.size,
                                                         'bg-gold' : store.state.selectedCatalogs.get(catalog.id) === info.size,
                                                ...
                                                >
                                                    <div :class="{ 'invisible lg:hidden' : catalog.catalogInfos.length === 1 }">
                                                        {{ info.size }}
                                                    </div>
                                                    <div :class="{ 'h-full transform -translate-y-1/2 flex items-center justify-center lg:translate-y-0' : catalog.catalogInfos.length === 1 }">
                                                        {{ info.price }}
                                                    </div>
                                                </div>
